...
import { getListUserAsync, setCurrentPage, setOpenCreate, setPageSize, setSearchKeyword } from './slices';
  ...
  return (
    <div>
      ...
      <div className='flex items-center justify-between'>
        <p className='text-[18px] font-bold'>{t('user_management')}</p>
      ...
      <div className="relative border border-gray-300 flex items-center mx-auto rounded-lg max-w-md h-10  mt-3 mb-5">
        <div style={{ width: '85%' }} className='flex items-center justify-between'>
          ...
          />
          {keyword && (
            <CiCircleRemove onClick={handleClearClick} className='mr-2 cursor-pointer' />
          )}
        </div>

        <button onClick={handleSearchUser} id='button-search' style={{ width: '15%' }} className="h-10 flex justify-center items-center bg-blue-700 text-white text-[20px] absolute right-0">
          <IoSearchOutline />
        </button>
      </div>
                  ...
                  <TableCell key={colIndex} type={column.type}>
                    {column.title === 'action' ? (
                      <div className='text-[16px]'>
                        <button className='pr-3' ><FiEdit /></button>
      ...
      <Overlay isOpen={openCreate} onClose={() => dispatch(setOpenCreate())}>
        <div className='relative w-full sm:w-auto sm:max-w-md'><FormCreateUser onCancel={() => dispatch(setOpenCreate())} /></div>
      </Overlay>
    </div>
  );
};
