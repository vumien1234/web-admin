...
import { setCurrentPageBeforefly, setInitialData, setOpenCreate, setPageSizeBeforefly, setOpenDelete, setOpenEdit } from './slices';
import Overlay from '../../../components/common/Overlay';
import FormCreateBeforeFly from './FormCreateBeforeFly';
import FormEditBeforeFly from './FormEditBeforeFly';
  ...
  return (
    <div>
      ...
      <div className="relative border border-gray-300 flex items-center mx-auto rounded-lg max-w-md h-10  mt-3 mb-5">
        ...
        <button id='button-search' style={{ width: '15%' }} className="h-10 flex justify-center items-center bg-blue-700 text-white text-[20px] absolute right-0">
          <IoSearchOutline />
        </button>
      </div>
    ...
    <div className="overflow-x-auto">
                  ...
                  <TableCell key={colIndex} type={column.type}>
                    {column.title === 'action' ? (
                      <div className='text-[16px]'>
                        <button className='pr-3' onClick={() => handleOpenEdit(row)}><FiEdit /></button>
                        <button onClick={handleOpenDelete}><FaRegTrashCan /></button>
                      </div>
                    ) : (
                      row[column.title]
                    )}
      ...
      <Overlay isOpen={openCreate} onClose={() => dispatch(setOpenCreate())}>
        <div className='relative w-full sm:w-auto sm:max-w-md'><FormCreateBeforeFly onCancel={() => dispatch(setOpenCreate())} /></div>
      </Overlay>
      {/* Modal chỉnh sửa */}
      <Overlay isOpen={openEdit} onClose={() => dispatch(setOpenEdit())}>
        <div className='relative w-full sm:w-auto sm:max-w-md'>
          <FormEditBeforeFly initialData={initialData} onCancel={() => dispatch(setOpenEdit())} />
        </div>
      </Overlay>
      {/* Modal xóa */}
