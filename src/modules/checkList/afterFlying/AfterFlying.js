...
import { setOpenCreate, setOpenDelete, setOpenEdit, setInitialData, setPageSizeAfterfly, setCurrentPageAfterfly } from './slices';
...
const AfterFlying = () => {
  ...
  const { openEdit, openCreate, openDelete,page,pageSize } = useSelector((state) => state.afterFly); 
  ...
  return (
    <div>
      <div className='flex items-center justify-between'>
        <p className='text-[18px] font-bold'>{t('check_after_flying')}</p>
     ...
     <div className="overflow-x-auto">
                  ...
                  <TableCell key={colIndex} type={column.type}>
                    {column.title === 'action' ? (
                      <div className='text-[16px]'>
                        <button onClick={() => handleOpenEdit(row)} className='pr-3'><FiEdit /></button>
      ...
      <Overlay isOpen={openCreate} onClose={() => dispatch(setOpenCreate())}>
        <div className='relative w-full sm:w-auto sm:max-w-md'><FormCreate onCancel={() => dispatch(setOpenCreate())} /></div>
      </Overlay>
    </div>
  );
};
