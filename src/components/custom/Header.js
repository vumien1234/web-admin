...
import { MdHistory, MdPerson, MdLogout, MdOutlineLanguage } from 'react-icons/md';
    ...
    <Box sx={{ px: 2.5 }} className='wrapper-header'>
      ...
      <Overlay isOpen={isSearchOpen} onClose={handleCloseSearch}>
        <Box className='popup-search relative p-4'>
          ...
          <div>
            <p className='text-[12px] mt-2 text-gray-500'>{t('recently')}</p>
            <div id='item-history' className='flex items-center p-2 mt-2 h-[40px] border bg-gray-50 rounded cursor-pointer'>
              ...
              <p className='pl-3 text-[12px]'>{t('aircraft_management')}</p>
            </div>
          </div>
        </Box>
      </Overlay>
