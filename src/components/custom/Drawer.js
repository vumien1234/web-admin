...
const Drawer = () => {
	...
	const handleChangeScreen = (event, path, key, index) => {
		...
		if (path !== location.pathname) {
			...
			} else { // If it's a parent without children or a direct path
				setOpenSubMenuParent(null);
				setActiveParent(key);
				setIndexMenuActive(index);
				setActiveSubMenu(null);
	...
	return (
		<div className={`wrapper-drawer ${isDrawerOpen ? 'open' : 'closed'}`}>
			<Box sx={{ ml: 1.5 }} className="header-drawer">
				<img id="img-header" style={{ width: "40px", height: "35px" }} src={Logo} alt="image" />
				{isDrawerOpen && <p className="title-header">DRONE</p>}
			</Box>
				...
				<List>
					{paths.map((parent, index) => (
						<React.Fragment key={parent.key}>
							<ListItem
								onClick={(event) => handleChangeScreen(event, parent.to, parent.key, index)}
								...
								<ListItemText>
									<Box sx={{ display: 'flex', alignItems: 'center', ml: 0.5 }}>
										...
										{isDrawerOpen && <span style={{ fontSize: '14px' }}>{t(parent.category)}</span>}
									</Box>
								</ListItemText>
							</ListItem>
							{isDrawerOpen && openSubMenuParent === parent.key && parent.children && (
								<List>
										...
										<ListItem
											...
											<ListItemText>
												<Box sx={{ pl: 6, cursor: 'pointer' }}>
													<span style={{ fontSize: '14px' }}>{t(child.name)}</span>
												</Box>
											</ListItemText>
										</ListItem>
									))}
